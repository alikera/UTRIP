...
void Hotel::write_each_hotel_info(int index, string item)
{
	switch(index)
	{
		...
		case 4:
			...
			break;
		case 5:
			amenities_list = create_amenities_list(item);
			break;
		case 6:
			city = item;
			break;
...
amenities_t Hotel::create_amenities_list(string item)
{
	amenities_t my_list;
	stringstream ss_item(item);
	while(getline(ss_item, item, OR))
...
{
	double weighted_sum = calculate_weighted_sum(user_weights);
	double weights = calculate_weightes(user_weights);

	personal_rating = weighted_sum / weights;
}

double Hotel::calculate_weighted_sum(map_t user_weights)
{
	double weighted_sum = 0;

	for(auto weight : user_weights)
		for(auto rating : default_ratings)
			weighted_sum += calculate_weighted_rating(weight, rating);
...
double Hotel::calculate_weightes(map_t user_weights)
{
...
string Hotel::get_hotel_info_in_html()
{
	ostringstream body;
	body
		<< "<html>" << endl
		<< "<head><title>Hotel Infos</title>" << endl
		<< "<style>" << endl
		<< "table, th, td {" <<endl
		...
		<< "<body>" << endl
		<< "  <p>" << endl
		<< "      <img src='" << image_url << "'width='500' height='500' style='float:top'"
		<< "</p><br>" << endl
		<< "      <h3> property name: </h3>" << name << endl
		<< "      <h3> hotel overview: </h3>" << overview << endl
		<< "      <h3> property amenities: </h3>" << endl
		<< "		<ol>" << endl
		<< print_all_amenities();
	body<< "        </ol>" << endl
		<< "      <h3> hotel star rating: </h3>" << stars << endl
		<< "      <h3> city: </h3>" << city << endl
		<< "      <h3> rooms:"
		<< create_table()
		<< "</body>" << endl
		<< "</html>" << endl;
...
string Hotel::print_all_amenities()
{
	ostringstream amenities_body;	
	for (int i = 0; i != amenities_list.size(); ++i)
		amenities_body << "<li>" << amenities_list[i] << "</li><br>" << endl;

	return amenities_body.str();
}

string Hotel::create_table()
{
	ostringstream rooms_table;
	rooms_table << "<table style='width:100%'>" << endl
	  << "<tr>" << endl
   	  << " <th>&nbsp;</th> " << endl << "<th>Standard Room</th>" << endl
      << "<th>Delux Room</th>" << endl << "<th>Luxury Room</th>" << endl
      << "<th>Premium Room</th>" <<endl
      << "</tr>" << endl

      << "<tr>" << endl
      << " <th>Price</th> " << endl << "<td>" << rooms_detail.standard_price << "</td>" << endl
      << "<td>" << rooms_detail.delux_price << "</th>" << endl 
      << "<td>" << rooms_detail.luxury_price << "</th>" << endl
      << "<td>" << rooms_detail.premium_price << "</th>" <<endl
      << "</tr>" << endl

      << "<tr>" << endl
      << " <th>Number of Rooms</th> " << endl << "<td>" << standard_rooms.size() << "</td>" << endl
      << "<td>" << delux_rooms.size() << "</th>" << endl 
      << "<td>" << luxury_rooms.size() << "</th>" << endl
      << "<td>" << premium_rooms.size() << "</th>" <<endl
      << "</tr>" << endl
