...
public:
	...
	bool username_is_unique(const std::string& username);
	void process_logout(const std::string& username);

	bool have_empty_field(const std::string& username, const std::string& password, 
		 const std::string& email);
	bool username_and_email_are_unique(const std::string& username,
		 const std::string& email);
	void create_user(const std::string& username, const std::string& password, 
		 const std::string& email);

	hotels_t get_all_hotels(const std::string& session_id);
	Hotel* get_hotel(const int& id);
	void apply_star_filter(const int& min_star, const int& max_star,
		 const std::string& session_id);
	double get_user_wallet_credit(std::string session_id);
	void charge_user_wallet_credit(double amount, std::string session_id);
	bool user_is_logged_in(std::string session_id);
...
private:
	...
	bool email_is_unique(const std::string& email);
	...
	bool get_weights_input(std::stringstream& ss_inputs, map_t& weights, User* logged_user);
